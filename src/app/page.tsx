...
export default function HomePage () {
  ...
  const handleLogout = async () => {
    try {
      ...
      router.push('/') // Optional: send back to home
    } catch (err) {
      console.error(err)
    }
  }
  ...
  return (
    <main className='relative min-h-screen bg-gradient-to-br from-sky-400 via-rose-100 to-blue-200 flex items-center justify-center p-6'>
      {/* Logout (top-right) */}
      {userId && (
        <button
          onClick={handleLogout}
          className='absolute top-6 right-6 p-3 rounded-full bg-gradient-to-r from-red-600 to-red-500 text-white shadow-lg hover:brightness-110 active:scale-95 focus:outline-none focus:ring-4 focus:ring-red-200'
        >
